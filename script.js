...
const setupScrollDots = ({ container, items, label }) => {
  ...
  dots.setAttribute("aria-label", `${label}滑动分页`);

  const buttons = items.map((_, index) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "scroll-dot";
    button.setAttribute("aria-label", `${label}第 ${index + 1} 页`);
  ...
  const setActive = () => {
    ...
    const activeIndex = Math.min(items.length - 1, Math.max(0, Math.round(container.scrollLeft / pageWidth)));
...
setupScrollDots({ container: document.querySelector(".tab-panels"), items: tabPanels, label: "购物" });
...
setupScrollDots({ container: document.querySelector(".hotel-panels"), items: stayPanels, label: "酒店" });
